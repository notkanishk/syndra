...
| [Add-on Transport: Derived Keys](changes/addon-transport-derived-keys/) | 4 | Proposed — sequenced behind the live TrueNAS bring-up | [proposal](changes/addon-transport-derived-keys/proposal.md) / [design](changes/addon-transport-derived-keys/design.md) / [tasks](changes/addon-transport-derived-keys/tasks.md) — one per-target secret replaces the private CA; the add-on's Ed25519 TLS key is HKDF-derived and pinned, not distributed. The certificates were answering confidentiality, not authentication, and `secret_params` is why the TLS stays |
