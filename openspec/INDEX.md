...
| [Add-on Shutdown Grace Period](changes/addon-shutdown-grace-period/) | 4 | Proposed — a live defect, independent of the transport change | [proposal](changes/addon-shutdown-grace-period/proposal.md) / [tasks](changes/addon-shutdown-grace-period/tasks.md) — the add-on's 20s drain is killed by Docker's 10s default on every stop, so an in-flight mutation is abandoned in exactly the state the drain exists to prevent |
