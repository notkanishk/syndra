...
- **Commission the target plane's missing design** (`design_handoff_syndra/target-plane/`). `/system/targets/{target}` was drawn twice — §21 on desktop and M20 on touch, both as *four questions about one add-on* — and now carries eleven panels with no organising structure. Four of the eleven were designed individually; the composition never was. Six further surfaces appear on no board at all: the Connected systems index, merge findings and their decision form, the target's own system health, the managed roster, the two findings that render inside Health, and the applied history on a removed grant. The handoff is written and unsent: `PROMPT-1` (design system, self-contained), `PROMPT-2` (the redesign), `PROMPT-3` (the six undrawn screens), plus `REFERENCE-current-page.md` as an attachment. Prompt 2 is the one that matters and 3 inherits its answer.
