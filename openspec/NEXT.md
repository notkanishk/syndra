...
- **One TrueNAS API key** (`addon-transport-derived-keys` 7.4a) — everything else in the bring-up is done and verified on the live dev deployment (2026-08-13): the secret mints itself, the two binaries agree on the derived key, the manifest reads over the pinned channel, and the add-on reads a target's version end to end. What is left is a credential nobody but the operator can create: `nas.example.org` is TrueNAS and answers, but the deployment's key is a stub key and the real NAS rejects it. Mint one per DEPLOY.md "Bringing up the TrueNAS add-on" step 1 — group → privilege (`ACCOUNT_WRITE` + `SYSTEM_AUDIT_READ`, never `FULL_ADMIN`) → user → API key with an expiry — put it in `TRUENAS_API_KEY`, point `TRUENAS_URL` at `wss://nas.example.org/api/current`, and `docker compose up -d`. **`wss://` only**: TrueNAS revokes a user-linked key presented over plaintext, so a `ws://` typo destroys the credential rather than failing.
- **A drain with a mutation actually in flight** (`addon-shutdown-grace-period` 3.3) — the stop itself is now observed live: the drain runs, the process exits itself in ~1s against a 30s grace, exit 0, no SIGKILL. The unobserved half needs a target call slow enough to stop inside, which the stub cannot provide.
