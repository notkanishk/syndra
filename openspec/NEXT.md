...
- **BIA-38** — ~~field hints written inside their `<label>`~~ **done
  2026-08-27.** There were eight, not the three a cruder grep suggested. Swept
  with the rest of `one-control-surface` group 8, and guarded against the
  source so a ninth cannot be written.
...
- **Build the target plane's redesign** (`design_handoff_syndra/target-plane/`). The commission came back on 2026-08-27: two boards, twenty figures (`T1`–`T5`, `S1`–`S15`), a prose reply and an inventory that verifies against them. `BUILD-NOTES.md` is the reconciliation and overrides the reply where they disagree.

  **The structural finding:** health and maintenance are one question — *not answering* while somebody is draining it for a credential rotation is a different fact from *not answering* at 04:00 — and the built page has them six panels apart. The redesign is a band (Syndra's reach, the target's own readings, the maintenance state) then three regions: waiting on a person, people and their access, what runs here. Panels 3 and 4 leave for a mapping screen that does not exist yet, so **nothing may be removed from the built page until it does**.

  **One drawn promise the backend refuses:** the decided-and-waiting row says deciding again replaces the queued work. `RecordMergeDecision` guards on `decision IS NULL` and fails closed, because the two resolutions it sits between are recreate-the-account and stop-managing-it. Invert the copy; do not relax the guard.

  Still open: what routes sit behind *apply it again* / *stop wanting it*; which index readings can co-occur; and that a member reads nothing when an operator sets read-only — `MyStorage` carries no lifecycle field, in neither backend nor UI.
