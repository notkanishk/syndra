...
- **The TrueNAS bring-up, on the derived transport** (`addon-transport-derived-keys` 7.4) — the first one, and it now carries a transport that has never handshaked outside a test. Run `./scripts/smoke-test-addon.sh truenas` FIRST: it checks the secret's mode and ownership and diffs the key the backend pins against the key the add-on serves, which are the two legs that are Syndra's own. It stops before the NAS deliberately — diagnosing that leg together with the transport is what the sequencing existed to prevent, and the sequencing is the thing that did not happen.
