...
- **Expiring-access acknowledgement (C4)** — deferred, with the question written down rather than left implicit: a server-backed, audited row carrying who, when and an optional rationale, and — the load-bearing part — a rule for **when it reopens** (grant changed / timer / manual only). Undecided. Build it when there is a queue big enough to have an opinion.
