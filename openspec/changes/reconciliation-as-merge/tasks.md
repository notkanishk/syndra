...
- [ ] 1.1 A `merge_base` per `(subject, target)`, written from the observed values of 0.2. Intent MUST NOT be recorded as the base: a base equal to `OURS` by construction can never produce a conflict, which is today's behaviour with more machinery
