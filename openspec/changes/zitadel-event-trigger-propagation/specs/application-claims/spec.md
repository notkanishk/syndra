...
* **Event-trigger target (`mkauth-event-listener`)**: type MUST be `restAsync` (fire-and-forget — Zitadel does not block the originating actor on MkAuth latency); endpoint MUST be `${MKAUTH_EXTERNAL_URL}/api/webhooks/zitadel`; executions MUST cover `condition.event` for at minimum `user.human.added`, `user.human.deactivated`, `user.human.locked`, `user.grant.added`, `user.grant.changed`, `user.grant.removed`. Self-registration (`user.human.selfregistered`) MAY also be bound when self-service signup is enabled.
...
* **Translator coverage**: the translator MUST map the following Zitadel events into internal `event_type` values, populating per-event fields (`user_id`, `source_project`, `role_keys[]`):
    ...
    * `user.human.deactivated` → `user_deactivated`
    * `user.human.locked` → `user_locked`
