...
* Adds a second Zitadel Actions v2 target `mkauth-event-listener` (type `restAsync`, endpoint `/api/webhooks/zitadel`) bound to `condition.event` triggers for `user.human.added`, `user.deactivated`, `user.locked`, `user.grant.added`, `user.grant.changed`, `user.grant.removed`. Deactivation and lock are user-aggregate events, not human-aggregate (Zitadel registers them as `userEventTypePrefix + "deactivated"` / `+ "locked"` in `internal/repository/user/`); using `user.human.deactivated` / `user.human.locked` triggers `COMMAND-74aaqj8fv9` "Execution condition is invalid" at SetExecution time.
