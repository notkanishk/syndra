...
- **Purge runs on a second credential the backend injects.** ~~The add-on's privilege excludes `user.delete`.~~ **It does not, and cannot.** TrueNAS requires `ACCOUNT_WRITE` for `user.delete` — the same role `user.create` and `user.update` need — and publishes no narrower one ([RBAC reference](https://api.truenas.com/v25.10/rbac.html)), so the standing key can delete an account and a compromised add-on can too. What the injected credential buys is real but smaller than this bullet claimed: no delete travels on the long-lived session, the elevated key is never stored by the add-on, and every deletion is attributable to a credential issued for that single call. An **audit and blast-radius** separation, not a capability one. Corrected after the claim survived three reviews on the strength of sounding right — see `tasks.md` §32.4.
...
Add-on calls carry a timestamp and a body hash, signed, over a TLS connection whose server key the backend pins. A bare shared secret is not sufficient: it authenticates the caller but binds nothing to the request, so an intercepted call can be replayed verbatim.

**This section described two modes — mTLS with a private CA, or signed requests — until `addon-transport-derived-keys`.** There is one mode now, and both of its keys are derived with HKDF from a single per-target secret; nothing is distributed and nothing expires. Read that change's design for the reasoning; what survives here unchanged is every property below, because they were never properties of the certificates.

**The base URL must be https, and that is not belt-and-braces over the above — it is what makes the above happen.** A client's TLS settings are consulted only where a handshake occurs, so an `http://` base URL means the add-on's key is never pinned. Signing needs TLS for the two properties a signature does not provide: over plaintext the secret-bearing body is readable by anything on the Compose network, and an on-path peer can forge a 2xx the backend records as a completed mutation — or forge a capability set. The declared `secret_params` are the reason this guard survives the transport change, and the reason its comment had to be rewritten: it used to say the check existed because a client certificate would never be presented, which is a *true statement about a thing that no longer happens* and would have invited the guard's removal. There is no localhost exemption, because an exemption for development is an exemption that ships.
...
**The pin is the trust anchor, and there is no fallback under it.** `InsecureSkipVerify` is set and `VerifyPeerCertificate` compares the add-on's public key to the one derived from the deployment secret: two checks (a chain to some authority, and a name match) are replaced by one stronger question — does this peer hold the deployment's secret? A name check asks whether an authority vouched for a string. Between two components of one deployment the second question is strictly the stronger, and it is what stops an on-path attacker reaching a body carrying declared `secret_params`: the handshake fails before any body is written. A target configured with no secret does not register at all — calling the component that holds the target credential over an unauthenticated channel is worse than not having it.
...
- **The add-on is the least trusted component and holds the target credential** → the manifest is a ceiling over backend policy, never an authorization source (§13); the pinned, signed transport binds the channel (§16); the mutation log is tamper-evident (§17).
...
- **The add-on's credential is broad** → `ACCOUNT_WRITE` plus `SYSTEM_AUDIT_READ` is most of what matters on the NAS, **including deletion**, which TrueNAS does not let this be scoped below. Scoped as tightly as the feature set allows, key expiry set and surfaced, read-only kill switch available without redeploy. Honest limit: scoping buys less once ACL and quota writes arrive in phase 2 — and it already bought less than the purge bullet above claimed.
