...
- **Username derivation.** Deterministic from the Zitadel identity (email localpart plus collision suffix) makes the binding reconstructible if the add-on's store is lost; sequential assignment does not. Leaning deterministic, but it interacts with the mount-instruction UX.
