...
- [x] 2.21 Plan store secret exclusion: `secret_params` values are excluded from plan persistence; the value rides the apply request and is discarded with it. `outcome_json` is JSONB and would accept anything, so the guarantee cannot be that writers are careful — it is that no caller holds a route. `PlanOutcome` is a closed struct of strings the backend decided, with no map, no `any`, and nothing a submitted parameter set could be assigned to; the realistic mistake is not malice but an apply that already holds `params` adding a field so the plan can "show what will be sent". Name and email are absent for the adjacent reason: they are read from the directory at render time, and persisting them would make a plan a stale second copy of a profile
- [ ] 2.22 Tests: the plan types are asserted by reflection to expose no open-ended field, which is the structural half. The live-row scan — write a plan for an operation declaring a secret and grep the row, its indexes, and any cache for the submitted value — lands with the apply path in group 3, where there is a value being carried to scan for
