...
- [x] 1.4 Migration: `desired_state_snapshots` — immutable rows per `(subject, target)` with a monotonic version; add-on rows carry their intent here rather than in the outbox's Zitadel columns. Immutability is a `BEFORE UPDATE OR DELETE` trigger and monotonicity a `BEFORE INSERT` one that allocates `MAX(version)+1` per `(subject, target)`, not conventions — `UNIQUE` alone permits version 2 followed by version 1, which makes "older than the last version applied" compare against a number that went backwards
