...
* Adds a `sync/` directory containing an independent Go binary deployed as a separate Docker container alongside the existing backend, UI, Postgres, and Redis services.
...
* Shadow password sync: when processing any intent for a user, the worker checks `GET /api/v1/shadow-credentials/{uid}/hash` and syncs the hash to LLDAP if present.
...
* Adds sync service to `docker-compose.yml` (no public ports).
