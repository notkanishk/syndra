...
- `seed/demo.go` — `EnsureDemoData` short-circuits user/project seeding when live Zitadel is detected. DB-owned seed items (bundles, mapping rules, audit examples, access requests) still seed only when `SYNDRA_SEED_DEMO=true` is set; the default becomes *off in live mode, on in demo mode*.
...
- **Living docs updated**: `openspec/INDEX.md`, `openspec/changes/syndra-core-architecture/ROADMAP.md` (Phase 5 bullet), `openspec/changes/syndra-core-architecture/specs/feature-coverage.md`.
