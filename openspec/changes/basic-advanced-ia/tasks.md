...
- [x] BIA-42 **P2, from review of BIA-41** — the fix left `MEMBER_ROUTES` a hand-written second representation of `MEMBER_NAV`, guarded only by an equality test, which catches drift when somebody runs the tests rather than preventing it. Both are derived now: `MEMBER_ROUTES` is `navLeaves(MEMBER_NAV).map(href)` and `memberMayVisit` is `leafMatches` over the same leaves, so the middleware's rule is the rail's rule — including a leaf's `pattern` if a member destination ever gains a detail route, which the hand-rolled prefix check would have ignored. The equality assertion became a tautology and was replaced by a literal canary: widening member reach should fail a test deliberately, not pass because the list it was compared against moved too. Verified by adding a fourth member row — it is reachable with nothing else edited, and only the two count assertions fail.
