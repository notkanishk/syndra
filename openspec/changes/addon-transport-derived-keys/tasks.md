...
- [x] 7.4a **Against the REAL NAS (`nas.example.org`, TrueNAS-25.10.5), 2026-08-13.** An operator minted a key on a dedicated user whose group carries `ACCOUNT_WRITE` + `READONLY_ADMIN` + `SYSTEM_AUDIT_READ`; the add-on authenticated over `wss://`, read the version, and answered **all four** health reads with nothing degraded — so `system.info`, `alert.list`, `pool.query` and `service.query` are all granted by that privilege. `user.query` works too: the inventory read returned the NAS's real accounts, including one unmanaged (`sai`, uid 3000). `audit.query` and `sharing.smb.query` are NOT yet exercised — they belong to `activity.get`, which has no operator route on the backend, so the roles are configured and unproven rather than verified
- [x] 7.4c **The WRITE path, proven end to end against the real NAS (2026-08-18).** plan → apply (create) → password.set → account.purge, driven by signed requests exactly as the backend drives them, on a throwaway account that was verified present on the NAS between the steps and verified gone after. All three writes are in the tamper-evident mutation log with the actor recorded (`apply.create`, `password.set`, `account.purge`, seq 8–10). The reverse direction runs on schedule and reads the target back: `[ADDON-RECONCILE] truenas bound=3 queued=3 unmanaged=1 halted=false`
- [x] 7.4d **Account creation was broken against EVERY supported release, and only the real target could say so.** `user.create` sent neither `password` nor `password_disabled`; TrueNAS requires one. Three coupled rules were learned by being refused — see `truenas_rules_test.go`, which pins each with the target's own error text. The recorded fixtures answered that payload with a success, so the fake, the parser and both suites agreed with each other while the feature could not create a single account
- [x] 7.4b **The real target found a defect the stub never could.** `system.version` returns `TrueNAS-25.10.5`, not the bare `25.04.2.1` this suite's recorded fixture carries. `majorOf` split on "." from the left, produced `TrueNAS-25.10`, matched no entry in `TRUENAS_SUPPORTED_MAJORS`, and **refused every mutation against a release that is explicitly supported** — reporting "TrueNAS TrueNAS-25.10 is outside the tested range", naming a major nobody has ever written down. Reads all kept working, so the symptom was a silent capability loss rather than an outage. The parser now finds the first `N.N` anywhere in the string; fixed, tested against the four real prefixes, and re-verified live (`version_tested: true`)
...
## 10. The member's own view, which was the thinner half

- [x] 10.1 **A member is told when their account will not let them in.** Syndra creates the account before any password exists, and the target disables password authentication until the member sets one — so the page showed an account name, working mount instructions and a completed spine to somebody whose every connection attempt was refused. Read from the TARGET, because Syndra's record that a password was set cannot say whether the target still accepts it, and that gap is exactly where the failure lives
- [x] 10.2 **A held account gets a different sentence.** Telling somebody to set a password that will not help is worse than saying nothing
- [x] 10.3 **Usage, the most-asked storage question, which was unanswerable.** `storage.status` is a member-scoped read taking NO subject parameter, so there is no shape in which one member asks about another. Dispatched like a health read rather than an operation: it runs on an ordinary page load, and a durable row per view would fill the operation log with events that changed nothing
- [x] 10.4 **The dataset behind a share is derived from the share's own path**, never configured — a configured one is a second definition of where a share lives, and the two disagree the first time somebody moves one. Quota rows are matched on uid and never on the quota table's `name`, which is null for a uid the target can no longer resolve
- [x] 10.5 **No quota renders as "no limit set", never as a full bar.** TrueNAS omits the field entirely when none is set, which is the common case, and drawing 100% for "you are fine" would be the most alarming possible way to say it
- [x] 10.6 **The share host is set, and the reasoning behind it corrected.** `TRUENAS_SHARE_HOST=nas.example.org`. The docs asserted that name was a reverse proxy and therefore wrong for SMB; it is a direct DNS record, so the same name serves the API and tcp/445. The variable stays separate from `TRUENAS_URL` in both cases, because whether they match is a fact about somebody's network rather than something readable off a URL
