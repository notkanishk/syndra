...
Immediate priority note: the Phase 2 contract hardening wave is now complete for strict JSON request decoding, stable validation error envelopes, database-level invariants, and backend-first unit/handler regression coverage across mapping rules, grants, access requests, claims, and webhook boundaries.
...
| `application-claims`| Multi-project role selection, JWT claim shaping (array, CSV, space-delimited), simulator. | **Integrated** | `backend/internal/services/views.go`, `backend/internal/services/views_test.go`, `ui/src/app/applications/page.tsx`, `openspec/changes/mkauth-core-architecture/specs/application-claims/spec.md` | Simulation is powered by Redis-cached claims; claim-format regression tests now lock array/CSV/space-delimited contract fidelity. |
...
| **Webhook invalidation** | Webhook listener updates Redis on out-of-band Zitadel changes. | **Partial** | `backend/internal/handlers/webhook.go`, `backend/internal/cache/compiler.go`, `backend/internal/handlers/contracts_test.go` | Request schema is now strictly validated with stable error responses; real-world signature/authenticity validation and live sync remain for later integration work. |
...
With Phase 2 complete, the next priority is orchestration hardening in Phase 3: live Zitadel management API integration, production-grade webhook authenticity validation, and per-admin backend authorization boundaries beyond shared API-key development mode.
