...
| **Zero-trust backend vs UI** | Backend validates logged-in admin permissions before mutations. | **Not integrated** | (no per-admin identity enforcement found) | Current model is API key bearer auth; no user session + role-check enforcement yet. |
...
| **Access requests / governance view** | Pending requests, expiring grants, cleanup hints; least-privilege summary. | **Integrated** | `ui/src/app/requests/page.tsx`, `ui/src/app/audit/page.tsx`, `backend/internal/services/views.go` | Cleanup hints exist in governance summary; may still be heuristic/demo-driven. |
