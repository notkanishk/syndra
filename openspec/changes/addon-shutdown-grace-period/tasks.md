...
- [x] 3.3a **A mutation actually in flight, settling rather than being abandoned.** `shutdown_inflight_test.go` runs the real server main.go builds — its routes, its authenticator, its lifecycle — with a target call that has not answered yet, which is what produces the window in production too. It asserts the four things no budget check can: `Drained()` is false with a write outstanding, a second mutation arriving mid-drain is refused rather than queued behind it, `Shutdown` does NOT return while the handler is inside the target call, and once released the apply completes 200 and writes its terminal record. Mutation-verified by swapping `Shutdown` for `Close`, which abandons the write and fails the test. What this still does not cover is a real NAS being slow — everything on this side of that call is now covered
- [~] 3.3 **Half observed live (2026-08-13), half still gated.** `docker compose stop truenas-addon` on the dev deployment: the drain ran (`[SHUTDOWN] draining` then `[SHUTDOWN] stopped`), the process exited itself in ~1s against a 30s grace, exit code 0, not OOM-killed and not SIGKILLed — so the truncation this change exists to remove is gone on a real stop. What is NOT yet observed is the part that needs a mutation actually in flight: the target stub answers in milliseconds, so there is no window to stop inside, and manufacturing one would be testing the harness. Still owed against a real NAS with a slow call. Original row: Observed once against the live deployment: stop the add-on with a mutation in flight and confirm the settle completes and the terminal status is written. This is the only step that proves the original defect is gone rather than merely reconfigured — everything above asserts the numbers, not the behaviour
