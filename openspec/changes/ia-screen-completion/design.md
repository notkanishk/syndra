...
- ~~**Apps read one project each.**~~ **Since settled as the rule, not a workaround** (`ui-capability-gap-closure` Decision 14). The design drew an app reading four projects in two formats; Zitadel's model puts an application inside exactly one, and `app_claim_overrides.application_id` is unique, so the many-to-many the design assumed does not exist in the data. The index warns when a *project* is read in more than one format — the same failure expressed in the shape the data supports, and now the intended shape rather than a concession.
- ~~**Audit has no cascade id.**~~ **Since closed** (`ui-capability-gap-closure` Decisions 12–13). `audit_logs.cascade_id` is stamped inside `enqueueCascadeRows`, which was already minting the id; the trace column links to the named cascade, and rows written before the column keep their object id unlinked rather than being backfilled by guesswork.
