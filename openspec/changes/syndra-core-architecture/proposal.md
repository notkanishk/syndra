...
Syndra has moved beyond the original bundle-and-mapping prototype into a usable admin surface with seeded demo data, access requests, governance summaries, and a topology graph. The docs need to reflect the actual product shape so future work is built against the same contract the app now exposes.
