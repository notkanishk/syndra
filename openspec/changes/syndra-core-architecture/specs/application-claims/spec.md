...
- **Multi-project resolution**: when `user_grants` names more than one project, the response MUST emit namespaced claim keys `syndra.<projectID>.<claim>` so claims across projects cannot collide in the issued token. Single-project responses use flat keys so legacy single-audience applications receive the unprefixed payload.
...
- **THEN** Syndra MUST return the documented safe fallback envelope for that application
...
- **WHEN** Syndra introduces or changes an internal payload between the UI, backend, or sync service
...
- **THEN** Syndra MUST require a documented degraded-mode posture for cache miss, timeout, malformed cache data, or unavailable dependencies
...
The Zitadel Actions v2 target configuration and deployment assets MUST be maintained in the Syndra repository with operator documentation.
...
- **AND** Syndra MUST NOT schedule rotation automatically — the command MUST be invoked by explicit operator or external-trigger action.

## Rotation Status MUST be Observable to Operators
Because Zitadel does not expire the signing key and Syndra does not schedule rotation, operators need an in-app signal that a key has crossed the deployment's rotation-policy threshold. Syndra MUST expose `GET /api/v1/zitadel/action-rotation-status` (gated by `withOperatorAuth`) returning `{last_rotated_at, age_days, threshold_days, status, rotate_command}` and MUST render a corresponding panel on `/zitadel` in the admin UI.
