// Command syndra-token prints a freshly minted Zitadel M2M access token to
...
//
//	cd backend
//	ZITADEL_DOMAIN=... ZITADEL_MACHINE_KEY_PATH=... go run ./cmd/syndra-token
//
// Exit codes:
//
...
// an operator host that does not have the Syndra runtime dependencies
...
func main() {
	...
	if domain == "" {
		fmt.Fprintln(os.Stderr, "syndra-token: ZITADEL_DOMAIN is required (set in .env or export)")
		os.Exit(1)
	}
	if keyPath == "" {
		fmt.Fprintln(os.Stderr, "syndra-token: ZITADEL_MACHINE_KEY_PATH is required (set in .env or export)")
		os.Exit(1)
	}
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "syndra-token: %v\n", err)
		os.Exit(1)
	}
