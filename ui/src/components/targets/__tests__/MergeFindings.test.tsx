...
describe("differences waiting on a decision", () => {
  ...
  // The refusals are the useful half: adopting a group value has nowhere to
  // live, and the backend says so with the policy named. Swallowing that leaves
  // an operator pressing a button that never works.
  it("renders the refusal in full", () => {
    state.findings = [finding({ field: "group", theirs: ["electronics"] })];
    state.error = new Error(
      "group comes from this target's role mappings, which have no per-person form — changing one changes it for every holder of that role",
    );
    renderFindings();
