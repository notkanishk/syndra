# Handoff: Syndra — Basic / Advanced information architecture

## Overview

Syndra is access management for an academic makerspace. The existing UI is functionally complete but organisationally chaotic — thirteen top-level admin links across six sidebar sections, with overlapping names.
...
The HTML uses a small custom runtime (`support.js`) purely so the design could be authored as one streaming document. Ignore it. Open `design/Syndra IA.dc.html` in a browser to view the board; read the markup for exact values.
...
Every route in the matrix is now drawn. Board section numbers refer to `design/Syndra IA.dc.html`.
...
**Partial-coverage notice, required.** `GET /api/v1/roles` calls `GetAllLocalRoles`, which returns only roles created through Syndra. An accent-tint card above the table states: *"Showing Syndra-managed roles only. Roles created directly in the identity provider aren't listed yet."* with a link to check a project's roles upstream. Silently partial lists are how people conclude a role doesn't exist and create a duplicate.
...
"syndra.laser.roles": ["trained", "maintainer"],
"syndra.studio.roles": "door,wiki-read",
"syndra.metal.roles": []
...
**Bulk bar.** `4 selected` · `Adopt 4 in Syndra` (accent fill) · `Mark 4 as owned elsewhere` (outline) · and a stated absence: *"Bulk revoke is deliberately not offered."*
...
**Row.** Checkbox (18px) · **Who** (186px — avatar, name, status like "Alumni · left Mar 2026") · **What they can get into** (250px — project/role plus a risk pill: red `Safety-gated · cuts metal`, amber `Safety-gated`, neutral `Role not in catalogue`) · **Why Syndra can't explain it** (flex, a full sentence naming the upstream actor and date) · **Found** (96px, red when oldest) · **Resolve** (300px, right-aligned, always in the same order: `Adopt` accent fill / `Revoke` red outline / `Owned elsewhere` outline).
...
- **If you adopt** — what Syndra records, who becomes granter of record, and that nothing changes upstream.
...
**Reconciliation tab.** Syndra ↔ provider diff. Columns: Project/role · Syndra (right) · Provider (right) · Difference (sentence) · Action. Two directions of drift, named differently:

- provider has extras → red count, `See in triage →`
- Syndra has records the provider lacks (a write that never landed) → amber count, `Re-push →`
...
**Health is a sentence with a cause and a timestamp — never a green or red dot on its own.** Red card when down: *"Unreachable — 6 consecutive failures. Last successful call 09:38 today. Connection refused on `:8080/oauth/v2/token`. Syndra is serving its own cache; the degraded banner is showing everywhere."*
...
2. **`/roles` is only partially backed** — `GetAllLocalRoles` returns Syndra-created roles only. Ship the explicit scope notice until an aggregate endpoint or per-project fan-out over `GET /api/v1/zitadel/projects/{id}/roles` exists.
...
| File | What it is |
|---|---|
| `design/Syndra IA.dc.html` | The full design board — every screen, state and annotation, §01–§18. Open in a browser. |
